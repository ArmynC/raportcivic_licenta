...
QString* g_app = nullptr;
QString* g_file_path = nullptr;
static bool g_logError = false;

std::unique_ptr<QFile> g_logFile = nullptr;
std::unique_ptr<QTextStream> g_logStream = nullptr;
...
void Log::setup(char *argv[], const QString &app, int level)
{
    ...
    g_file_path = new QString(logDir.filePath(logFileName));
    qInstallMessageHandler(messageHandler);
    qInfo() << "===================================================";
    qInfo() << "[Name]" << *g_app;
    qInfo() << "[Path]" << applicationPath;
    qInfo() << "[QtIteration]" << QT_VERSION_STR;
#ifdef WIN32
