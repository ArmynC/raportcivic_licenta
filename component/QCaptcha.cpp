...
QCaptcha::QCaptcha(QQuickItem *parent):QQuickPaintedItem(parent){
    ignoreCase(true);
    QFont fontStype;
...
void QCaptcha::paint(QPainter* painter){
    painter->save();
...
int QCaptcha::_generaNumber(int number){
    return QRandomGenerator::global()->bounded(0,number);
}

void QCaptcha::refresh(){
    this->_code.clear();
    for(int i = 0;i < 4;++i)
    {
...
bool QCaptcha::verify(const QString& code){
    if(_ignoreCase){
