...
QQrCodeItem::QQrCodeItem(QQuickItem* parent):QQuickPaintedItem(parent){
    color(QColor(0,0,0,255));
    bgColor(QColor(255,255,255,255));
    size(100);
    setWidth(_size);
    setHeight(_size);
    connect(this,&QQrCodeItem::textChanged,this,[=]{update();});
    connect(this,&QQrCodeItem::colorChanged,this,[=]{update();});
    connect(this,&QQrCodeItem::bgColorChanged,this,[=]{update();});
    connect(this,&QQrCodeItem::sizeChanged,this,[=]{
        setWidth(_size);
        setHeight(_size);
        update();
    });
...
void QQrCodeItem::paint(QPainter* painter){
    if(_text.isEmpty()){
        return;
    }
