...
QFrameless::QFrameless(QQuickItem *parent)
    : QQuickItem{parent}
{
    appbar(nullptr);
...
void QFrameless::onDestruction(){
    qApp->removeNativeEventFilter(this);
}

void QFrameless::componentComplete(){
    if(_disabled){
        return;
    }
...
#endif
    connect(this,&QFrameless::topmostChanged,this,[this]{
        _setWindowTopmost(topmost());
    });
    _setWindowTopmost(topmost());
}

bool QFrameless::nativeEventFilter(const QByteArray &eventType, void *message, QT_NATIVE_EVENT_RESULT_TYPE *result){
#ifdef Q_OS_WIN
...
bool QFrameless::_isMaximized(){
    return window()->visibility() == QWindow::Maximized;
}

bool QFrameless::_isFullScreen(){
    return window()->visibility() == QWindow::FullScreen;
}

void QFrameless::_showSystemMenu(QPoint point){
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    DWORD style = ::GetWindowLongPtr(hwnd,GWL_STYLE);
...
bool QFrameless::_containsCursorToItem(QQuickItem* item){
...
bool QFrameless::_hitMaximizeButton(){
...
void QFrameless::_setMaximizePressed(bool val){
    _maximizeButton->setProperty("down",val);
}

void QFrameless::_setMaximizeHovered(bool val){
    _maximizeButton->setProperty("hover",val);
}

void QFrameless::_updateCursor(int edges){
    switch (edges) {
    case 0:
...
void QFrameless::showFullScreen(){
    window()->showFullScreen();
}

void QFrameless::showMaximized(){
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd,3);
#else
...
void QFrameless::showMinimized(){
    window()->showMinimized();
}

void QFrameless::showNormal(){
    window()->showNormal();
}

void QFrameless::setHitTestVisible(QQuickItem* val){
...
void QFrameless::_setWindowTopmost(bool topmost){
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    if(topmost){
...
bool QFrameless::eventFilter(QObject *obj, QEvent *ev){
#ifndef Q_OS_WIN
    switch (ev->type()) {
