...
Popup {
    ...
    enter: {
        if(d.position === QSheetType.Top){
            return enter_top
        }else if(d.position === QSheetType.Left){
            return enter_left
        }else if(d.position === QSheetType.Right){
            return enter_right
        }else{
            return enter_bottom
        }
    ...
    Item {
        ...
        onWinChanged: {
            if(win instanceof QWindow){
                win.setHitTestVisible(container)
            }
        }
        property int position: QSheetType.Bottom
    ...
    background: Rectangle {
        ...
        color: QTheme.dark ? Window.active ?  Qt.rgba(38/255,44/255,54/255,1) : Qt.rgba(39/255,39/255,39/255,1) : Qt.rgba(251/255,251/255,253/255,1)
    }
    Page{
        id: container
        anchors.fill: parent
    ...
    function open(position = QSheetType.Bottom){
        control.x = 0
        control.y = 0

        d.position = position
        if(d.position === QSheetType.Top || d.position === QSheetType.Bottom){
