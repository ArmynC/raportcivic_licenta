...
Item {
    ...
    property QObject items
    property QObject footerItems
    property int displayMode: QNavigationViewType.Auto
    property Component autoSuggestBox
    property Component actionItem
    property int topPadding: 0
    property int pageMode: QNavigationViewType.Stack
    property QMenu navItemRightMenu
    property QMenu navItemExpanderRightMenu
    ...
    Item {
        ...
        property bool isCompact: d.displayMode === QNavigationViewType.Compact
        property bool isMinimal: d.displayMode === QNavigationViewType.Minimal
        property bool isCompactAndPanel: d.displayMode === QNavigationViewType.Compact && d.enableNavigationPanel
        property bool isCompactAndNotPanel: d.displayMode === QNavigationViewType.Compact && !d.enableNavigationPanel
        property bool isMinimalAndPanel: d.displayMode === QNavigationViewType.Minimal && d.enableNavigationPanel
        property color itemDisableColor: QTheme.dark ? Qt.rgba(131 / 255, 131 / 255, 131 / 255, 1) : Qt.rgba(160 / 255, 160 / 255, 160 / 255, 1)
        ...
        function handleItems() {
            var _idx = 0
            var data = []
            var comEmpty = Qt.createComponent("QPaneItemEmpty.qml")
            if(items) {
                for(var i = 0; i < items.children.length; i++) {
                    ...
                    data.push(item)
                    _idx++
                    if(item instanceof QPaneItemExpander) {
    ...
    Component.onCompleted: {
        d.displayMode = Qt.binding(function() {
            if(control.displayMode !== QNavigationViewType.Auto) {
                return control.displayMode
            }
            if(control.width <= 700) {
                return QNavigationViewType.Minimal
            } else if(control.width <= 900) {
                return QNavigationViewType.Compact
            } else {
                return QNavigationViewType.Open
            }
        })
    ...
    Connections {
        ...
        function onDisplayModeChanged() {
            if(d.displayMode === QNavigationViewType.Compact) {
                collapseAll()
            }
    ...
    Component {
        ...
        Item {
            ...
            Behavior on height {
                enabled: QTheme.animationEnabled && d.animDisabled
                NumberAnimation {
                    duration: 83
                }
            }
                ...
                Rectangle {
                    ...
                    QIcon {
                        ...
                        color: {
                            ...
                            return QTheme.dark ? "#FFFFFF" : "#000000"
                        }
                    }
                    color: {
                        if(!item_control.enabled) {
        ...
        RowLayout {
            ...
            QIconButton {
                ...
                onClicked: {
                    ...
                    if(pageMode === QNavigationViewType.Stack) {
                        var nav_stack = loader_content.item.navStack()
                        var nav_stack2 = loader_content.item.navStack2()
                        nav_stack.pop()
                        if(nav_stack.currentItem.launchMode === QPageType.SingleInstance) {
                    ...
                    } else if(pageMode === QNavigationViewType.NoStack) {
        ...
        Item {
            ...
            StackLayout {
                ...
                visible: {
                    ...
                    return QPageType.SingleInstance === nav_stack.currentItem.launchMode
                }
            }
    ...
    Rectangle {
        ...
        border.color: QTheme.dark ? Qt.rgba(45 / 255, 45 / 255, 45 / 255, 1) : Qt.rgba(226 / 255, 230 / 255, 234 / 255, 1)
        border.width: d.isMinimal || d.isCompactAndPanel ? 1 : 0
        color: {
            if(d.isMinimal || d.enableNavigationPanel) {
                return QTheme.dark ? Qt.rgba(61 / 255, 61 / 255, 61 / 255, 1) : Qt.rgba(243 / 255, 243 / 255, 243 / 255, 1)
            }
            return "transparent"
        }
        QShadow {
        ...
        visible: {
            if(d.displayMode !== QNavigationViewType.Minimal)
                return true
        ...
        Flickable {
            ...
            ListView {
                ...
                model: d.handleItems()
                boundsBehavior: ListView.StopAtBounds
                highlightMoveDuration: QTheme.animationEnabled && d.animDisabled ? 167 : 0
                highlight: Item {
                    clip: true
                    Rectangle {
                        height: 18
                ...
                delegate: QLoader {
                    ...
                    sourceComponent: {
                        if(model === null || !model)
                            return undefined
                        if(modelData instanceof QPaneItem) {
                            return com_panel_item
                        }
                        if(modelData instanceof QPaneItemHeader) {
                            return com_panel_item_header
                        }
                        if(modelData instanceof QPaneItemSeparator) {
                            return com_panel_item_separatorr
                        }
                        if(modelData instanceof QPaneItemExpander) {
                            return com_panel_item_expander
                        }
                        if(modelData instanceof QPaneItemEmpty) {
    ...
    Component {
        id: com_placeholder
        Item {
            property int launchMode: QPageType.SingleInstance
            property string url
        }
    }
        ...
        function stackPush() {
            ...
            if(page) {
                switch(page.launchMode) {
                    case QPageType.SingleTask:
                        while(nav_stack.currentItem !== page) {
                            nav_stack.pop()
                            d.stackItems = d.stackItems.slice(0, -1)
                        }
                        return
                    case QPageType.SingleTop:
                        if(nav_stack.currentItem.url === url) {
                            return
                        }
                        break
                    case QPageType.Standard:
                    default:
            ...
            if(pageIndex !== -1) {
                ...
                if(comp.status === Component.Ready) {
                    ...
                    if(obj.launchMode === QPageType.SingleInstance) {
                        nav_stack.push(com_placeholder, options)
